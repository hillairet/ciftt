...
def create_or_update_issues(
    ...
    """
    new_issues = [issue for issue in issues if isinstance(issue, NewIssue)]
    existing_issues = [issue for issue in issues if isinstance(issue, UpdatedIssue)]

    created_issues = create_issues(github_client, owner, repo_name, new_issues) if new_issues else []
    updated_issues = update_issues(github_client, owner, repo_name, existing_issues) if existing_issues else []
